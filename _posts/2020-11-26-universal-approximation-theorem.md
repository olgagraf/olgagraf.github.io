...
Clearly, $G=\\{\gamma(wx+b)\hspace{0.1cm}\|\hspace{0.1cm} w, b \in\mathbb{R} \\}$ forms a proper subspace of $C(I_1)$ and $\overline{G}\neq C(I_1)$. Therefore, $\gamma(wx+b)$ should satisfy the condition imposed by the theorems. As we've seen in our Riemann integral analogy, indeed $\int_{0}^{1} \gamma(wx+b)dx=0$ for all $w, b\in \mathbb{R}$, but $\int_{0}^{1} \gamma(g(x))dx$ might not be zero for arbitrary $g\in C(I_n)$.
